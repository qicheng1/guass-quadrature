...
mpf_t d[100],roots[N],ders[N],erro;
long int r,n;
...
int main(int argc, char *argv[])
{
	...
	FILE *fp,*fq;
	mpf_t value_of_zero1,value_of_zero2,der_of_zero1,der_of_zero2,temp,p0,p1,p2,w[N];
	...
	for(i=1;i<=nn;i++)
	{
		...
		newton(roots[i+1],0,temp,erro,i+1,m);
	}
	mpf_set_ui(w[0],0);
	for(i=1;i<=nn+1;i++)
	{
